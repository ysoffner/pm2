...
UX.describeTable = function(process) {
  ...
  safe_push(table,
            ...
            { 'cpu usage' : (process.monit ? (process.monit.cpu + '%') : 'N/A') },
            { 'memory usage' : (process.monit ? UX.bytesToSize(process.monit.memory, 3) : 'N/A') },
            { 'children processes' : typeof process.children === 'object' ? Object.keys(process.children).length - 1 : 0 },
...
UX.dispAsTable = function(list, interact_infos) {
  ...
  var app_head = stacked ? ['Name', 'mode', 'status', '↺', 'cpu', 'memory'] :
        ['App name', 'id', 'mode', 'pid', 'status', 'restart', 'uptime', 'cpu', 'mem', 'child proc', 'watching'];
  ...
  list.forEach(function(l) {
    ...
    if (l.pm2_env.pmx_module == true) {
      ...
      if (!stacked) {
        if (typeof l.children === 'object' && l.children !== null) {
          obj[key].push(Object.keys(l.children).length - 1)
        } else {
          obj[key].push(0)
        }
